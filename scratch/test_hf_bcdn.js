...
async function run() {
  try {
    ...
    // Get the stream URL by fetching the player page through HF
    const workerPlayerUrl = `${workerBase}?playerUrl=${encodeURIComponent(playerUrl)}`;
    console.log('1. Fetching player page through HF:', workerPlayerUrl);
    ...
    console.log('2. Fresh Stream URL:', streamUrl);

    // Fetch the stream URL through the Hugging Face Space
    const workerStreamUrl = `${workerBase}?streamUrl=${encodeURIComponent(streamUrl)}`;
    console.log('3. Fetching stream through HF:', workerStreamUrl);
    ...
    console.log('4. HF Response Status:', streamRes.status);
    console.log('   HF Response Headers:', Object.fromEntries(streamRes.headers.entries()));
    const body = await streamRes.text();
    console.log('   HF Response Body (first 200 chars):', body.slice(0, 200));
  } catch (err) {
    console.error(err);
  }
}
