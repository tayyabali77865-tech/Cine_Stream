...
async function testReferers() {
  try {
    ...
    const testCases = [
      { name: 'Derived Referer & Origin', headers: { 'Referer': urlObj.origin + '/', 'Origin': urlObj.origin } },
      { name: 'fmoviesunblocked Referer', headers: { 'Referer': 'https://fmoviesunblocked.net/' } },
      { name: 'netmirror.global Referer', headers: { 'Referer': 'https://netmirror.global/' } },
    ...
    for (const tc of testCases) {
      ...
      const res = await fetch(streamUrl, { headers: hdrs });
      console.log(`${tc.name}: Status = ${res.status} ${res.statusText}`);
    }
