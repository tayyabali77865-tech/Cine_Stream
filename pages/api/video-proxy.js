...
export default async function handler(req, res) {
  try {
    ...
    while (redirectsFollowed < maxRedirects) {
      ...
      // Try up to 4 random proxies from the pool
      ...
      for (let i = 0; i < Math.min(4, shuffleList.length); i++) {
        ...
        try {
          ...
          if (response.status === 206 || response.status === 200 || (response.status >= 300 && response.status < 400)) {
            ...
            console.warn(`[Proxy Rotator] Proxy ${proxy} returned ${response.status}`);
          }
        } catch (err) {
          console.warn(`[Proxy Rotator] Proxy ${proxy} failed:`, err.message);
        }
      }

      // Fallback: direct fetch if all proxies fail
      if (!success) {
